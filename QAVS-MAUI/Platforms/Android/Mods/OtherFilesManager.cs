...
namespace QuestAppVersionSwitcher.Mods
{
    public class OtherFilesManager
    {
        ...
        {
            ComputerUtils.Logging.Logger.Log("supported: " + String.Join(", ", type.SupportedExtensions));
            ComputerUtils.Logging.Logger.Log(type.Path);
            ComputerUtils.Logging.Logger.Log(type.NameSingular);
