...
import { IAPIDownloadsResponse, IDownloadManagerInfo, IGameDownloadItem, cancelGameDownload, getDownloads } from "../api/downloads";
...
export default function DownloadProgressPage() {
  const [gameDownloads, setGameDownloads] = createDeepSignal<IGameDownloadItem[]>([])
...
function GameDownloadItem(props: { download: IGameDownloadItem }) {
  ...
  const downloadStatus = createMemo<{
    ...
    if (state === StateOfDownload.Downloading && download.downloadManagers.length > 0) {
  ...
  return <div class="bg-[#111827] rounded relative overflow-hidden ">
    ...
    <div class="p-4 flex flex-row">
      <div class="flex-grow">
        <div class="flex gap-2 items-baseline">
          ...
          <div class="text-xs text-accent">{downloadStatus().download.version}</div>
        </div>
        <div class="text-xs ">
          <Show when={downloadStatus().state === StateOfDownload.Downloading && downloadStatus().manager}>
