    ...
    <PageLayout>
      <div class="contentItem">
        ...
        <Show when={config()?.currentApp && moddingStatus()?.isInstalled}>
          <RunButton
            text={`${moddingStatus()?.isPatched ? "Repatch" : "Patch"} ${GetGameName(config()!.currentApp)}`}
            icon={<FirePatch />}
            variant="success"
          ...
          <Box sx={{ marginY: 3 }}>
            ...
            <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
              <Switch
                checked={patchingOptions()?.externalStorage}
                onChange={() => {
                  updatePatchingOptions({
                    externalStorage: !patchingOptions()?.externalStorage,
                  });
                }}
              />
            ...
            <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
              <Switch
                checked={patchingOptions()?.debug ?? false}
                onChange={() => {
                  updatePatchingOptions({ debug: !patchingOptions()?.debug });
                }}
              />
            ...
            <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
              <Switch
                checked={
                  patchingOptions()?.otherPermissions?.includes(
            ...
            <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
              <Switch
                checked={patchingOptions()?.openXR ?? false}
                onChange={() => {
                  updatePatchingOptions({ openXR: !patchingOptions()?.openXR });
                }}
              />
            ...
            <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
              <Switch
                checked={patchingOptions()?.handTracking ?? false}
                onChange={() => {
                  updatePatchingOptions({
                    handTracking: !patchingOptions()?.handTracking,
                  });
                }}
              />
            ...
            <Box sx={{ marginY: 3 }}>
              ...
              <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
                <Select
                  ...
                  variant="outlined"
                  color="primary"
                  value={patchingOptions()?.handTrackingVersion ?? null}
                  onChange={(e) => {
                    updatePatchingOptions({
                      handTrackingVersion: e.target.value,
                    });
                  }}
                >
                  <MenuItem value={HandtrackingType.None}>
                    Default (Recommended)
                  </MenuItem>
            ...
            <Box sx={{ marginY: 3 }}>
              ...
              <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
                <Select
                  ...
                  variant="outlined"
                  color="primary"
                  value={patchingOptions()?.modloader ?? null}
                  onChange={(e) => {
                    updatePatchingOptions({ modloader: e.target.value });
                  }}
                >
                  <MenuItem value={ModLoaderType.QuestLoader}>
                    QuestLoader (Quest 2)
                  </MenuItem>
                  <MenuItem value={ModLoaderType.Scotland2}>
                    Scotland2 (Quest 3)
                  </MenuItem>
                </Select>
              </Box>
        ...
        <Show
          when={config()?.currentApp && moddingStatus()?.isInstalled === false}
        >
          <NoticeText>
            The selected game is not installed, install it from the oculus store
            or the downgrade section.
          </NoticeText>
          <NoticeText>
            If you installed the game and you still see this message press the
            button below
          </NoticeText>
          <RunButton
            text="Refresh"
        ...
        <Show when={!config()?.currentApp}>
          <NoticeText>
            No game selected, select your game from the left menu, if it's not
            there, install it first
          </NoticeText>
        </Show>
        <Show when={isPatchingModalOpen()}>
          <PatchingModal
            open={isPatchingModalOpen()}
            onClose={() => {
              setIsPatchingModalOpen(false);
            }}
            onPatchFinished={onPatchFinished}
          />
        </Show>
        ...
        <Box sx={{ marginY: 3 }}>
          ...
          <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
            <Switch
              checked={patchingOptions()?.resignOnly ?? false}
              onChange={() => {
                updatePatchingOptions({
                  resignOnly: !patchingOptions()?.resignOnly,
                });
              }}
            />
  ...
  return (
    <Box sx={{ marginY: 3 }}>
      ...
      <FileDropper
        ...
        overlayText={
          "Drag and drop an image, if you want transparent background use a png file"
        }
      ...
      >
        <Show when={patchingOptions()?.splashImageBase64}>
          <img
            src={patchingOptions()?.splashImageBase64}
            style={{
              width: "100px",
              "max-height": "100px",
              "object-fit": "cover",
 ...
 */
export function toggleAdditionalPermission(
  permission: string,
  enable?: boolean,
) {
  ...
  const filteredPermissions = options.otherPermissions.filter(
    (i) => i !== permission,
  );

  if (enable) {
    updatePatchingOptions({
      otherPermissions: [...filteredPermissions, permission],
    });
  } else {
