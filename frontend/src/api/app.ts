...
export async function changePort(port: number) {
  let result = await fetch(
    `/api/questappversionswitcher/changeport?body=${port}`,
  );
...
): Promise<IAppListItem[]> {
  let result = await fetch(
    includeBackups
      ? "/api/android/installedappsandbackups"
      : "/api/android/installedapps",
  );
  return await result.json();
}
 ...
 */
export async function setOculusToken(
  token: string,
  password: string,
): Promise<boolean> {
 ...
 */
export async function changeManagedApp(
  appId: string,
  appName?: string,
): Promise<boolean> {
  let data: {
...
/**
 * Fetches the given url through the proxy to avoid CORS issues
