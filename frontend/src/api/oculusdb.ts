...
export async function OculusDBGetGame(
  oculusId: string,
  test: boolean = false,
): Promise<{
  ...
  const response = await proxyFetch(
    `https://oculusdb.rui2015.me/api/v1/connected/${oculusId}?onlydownloadable=true`,
  );
  if (response.ok) {
...
export async function OculusDBSearchGame(
  gameId: string,
  test: boolean = false,
): Promise<IOculusDBApplication[]> {
  // if (test) {
  //     return searchData;
  // }

  let response = await proxyFetch(
    `https://oculusdb.rui2015.me/api/v1/search/${gameId}?headsets=MONTEREY,HOLLYWOOD,SEACLIFF`,
  );
