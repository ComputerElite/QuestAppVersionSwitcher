...
import { grantAccessToAppAndroidFolders, grantManageStorageAccess, gotAccessToAppAndroidFolders, hasManageStorageAccess } from "../../api/android";
    ...
    return (
        <div>
            ...
            <Show when={sdkVersion() >= 35}>
                <MediumText>
                    Your android version is not supported yet, proceed at your own risk <span >{GetAndroidVersionName(sdkVersion())}</span>
                </MediumText>
            </Show>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', mt: 2, gap: "5px" }}>
                <Show when={permissionsToGrant().includes(AndroidPermission.ManageStorageAccess)}>
                ...
                <Show when={permissionsToGrant().includes(AndroidPermission.AppDirAccess)}>
                    <RunButton disabled={appDirGranted()} text="Allows QAVS access to game dirs" onClick={grantAppDirAccess} />
                </Show>
            </Box>
