...
export function ValidateToken(token: string): {
  ...
  if (token.includes("%")) {
    return {
      isValid: false,
      message:
        "You got your token from the wrong place. Go to the payload tab. Don't get it from the url.",
    };
  }
  ...
  if (token.includes("|")) {
    return {
      isValid: false,
      message:
        "You seem to have entered a token of an application. Please get YOUR token. Usually this can be done by using another request in the network tab.",
    };
  }
  if (token.includes(":")) {
    return {
  ...
  if (/OC[0-9]{15}/g.test(token)) {
    return {
      isValid: false,
      message:
        "Don't change your token. This will only cause issues. Check another request for the right token.",
    };
  }
  return {
    isValid: true,
...
const _isOnQuest =
  ...
  "Mozilla/5.0 (X11; Linux x86_64; Quest) AppleWebKit/537.36 (KHTML, like Gecko) OculusBrowser/23.2.0.4.49.401374055 SamsungBrowser/4.0 Chrome/104.0.5112.111 VR Safari/537.36";
 ...
 * @returns
 */
export function DownloadImageAsPNG(
  url: string,
  fileName?: string,
): Promise<File> {
  return new Promise((resolve, reject) => {
